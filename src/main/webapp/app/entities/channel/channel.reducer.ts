...
const apiUrl = SERVER_API_URL + '/api/channels';

// Actions

export const getEntities: ICrudGetAllAction<IChannel> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_CHANNEL_LIST,
  payload: axios.get(`${apiUrl}?cacheBuster=${new Date().getTime()}`) as Promise<IChannel>
});
...
export const getEntity: ICrudGetAction<IChannel> = id => {
  ...
  return {
    ...
    payload: axios.get(requestUrl) as Promise<IChannel>
  };
};
